...
public class CollectDemo2 {
  public static void main(String[] args) {
    ...
    System.out.println("나라 이름 3개: " + names);

    sn = Nation.nations.stream();
    Set<String> names1 = sn.map(Nation::getName)
        .limit(3)
        .collect(Collectors.toSet());
    System.out.println("나라 이름 3개 (set): " + names1);

    sn = Nation.nations.stream();
    Map<String, Double> map = sn.filter(Nation::isIsland)
        .collect(Collectors.toMap(Nation::getName, Nation::getPopulation));
    System.out.println("섬나라 이름과 인구수 (map): " + map);

    sn = Nation.nations.stream();
    Set<Nation> set = sn.filter(Predicate.not(Nation::isIsland)) // 섬나라가 아닌 나라
        .collect(Collectors.toCollection(HashSet<Nation>::new)); // HashSet 생성
    System.out.println("섬나라가 아닌 나라 이름과 인구수: ");
